...
class ReservaServiceTest {
    ...
    @Test
    @DisplayName("Deveria atualizar reserva")
    void atualizarCenario01() {
        ...
        this.atualizarDTO = new AtualizarReservaDTO(
                usuariosTest,
                salaTest,
                LocalDateTime.now(),
                Status.ATIVA
        ...
        // ACTION
        reservaService.update(atualizarDTO, id);

        // ASSERT
        then(reservaRepository).should().save(reservaCaptor.capture());

        Reserva reservaSalvo = reservaCaptor.getValue();
        assertEquals(atualizarDTO.usuarios(), reservaSalvo.getUsuarios());
        assertEquals(atualizarDTO.sala(), reservaSalvo.getSala());
        assertEquals(atualizarDTO.inicio(), reservaSalvo.getInicio());
        assertEquals(atualizarDTO.status(), reservaSalvo.getStatus());
    }

    @Test
    @DisplayName("Deveria validar as regras de negócio")
    void atualizarCenario02() {
        ...
        validationAtualizarReserva.add(validationAtualizar);

        // ACTION
        reservaService.update(atualizarDTO, id);

        // ASSERT
        then(validationAtualizar).should().validation(atualizarDTO, id);
    }
}